...
public class Info {
	...
	public static String getselect(String name, String tablename, String zdname, String where) {
		...
		for (HashMap permap : new CommDAO()
				.select("select * from " + tablename + " where " + where + " order by id desc")) {
			String optionstr = "";
	...
	public static String getradio(String name, String tablename, String zdname) {
		String radio = "";
		for (HashMap permap : new CommDAO().select("select * from " + tablename + " order by id desc")) {
			radio += "<label><input type='radio' name='" + name + "' value=\"" + permap.get(zdname) + "\" />"
					+ permap.get(zdname) + "</label>\n";
		}
		return radio;
	}

	public static void writeExcel(String fileName, String prosstr, java.util.List<List> list,
		...
		try {
			response.sendRedirect("/sechandmall/upload?filename=" + fileName.substring(fileName.lastIndexOf("/") + 1));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static String getcheckbox(String name, String tablename, String zdname) {
		String checkbox = "";
		for (HashMap permap : new CommDAO().select("select * from " + tablename + " order by id desc")) {
			checkbox += "<label><input type='checkbox' name='" + name + "' value=\"" + permap.get(zdname) + "\" />"
					+ permap.get(zdname) + "</label>\n";
		}
		return checkbox;
	}
	...
	public static String getFileUpInfo() {
		String jscode = "";
		jscode += "<script src=/sechandmall/js/popup.js></script>";
		jscode += "<font onclick=\"uploaddoc()\" src=\"/sechandmall/js/nopic.jpg\" style='cursor:hand' id=txt >点击此处上传</font>";
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (!str1.equals("content")) {
					...
					jscode += "{\n";
					jscode += "document.getElementById(\"txt\").src=\"/sechandmall/upfile/" + map.get("filename")
							+ "\";\n";
					jscode += "}\n";
	...
	public static String getImgUpInfo(int height) {
		String jscode = "";
		jscode += "<img style=\"cursor: hand\" onclick=\"uploadimg()\" src=\"/sechandmall/js/nopic.jpg\" id=txt height=\""
				+ height + "\"/>";
		jscode += "<input type=hidden name=\"filename\" id=\"filename\" value=\"\" />";
		jscode += "<script type=\"text/javascript\"  src=\"/sechandmall/js/popups.js\"></script>";
		return jscode;
	}
