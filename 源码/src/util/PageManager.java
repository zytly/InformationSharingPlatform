...
public class PageManager {
	...
	protected PageManager(String path, int pageSize, HttpServletRequest request) {
		...
		try {
			this.currentPage = Integer.parseInt(request.getParameter("currentPage")) <= 0 ? 1
	...
	public void doList(String hql) {
		...
		if (this.count != 0) {
			this.pageNumber = count % this.pageSize == 0 ? this.count / this.pageSize : this.count / this.pageSize + 1;
			if (this.currentPage > this.pageNumber)
				this.currentPage = (int) this.pageNumber;

		}
		this.request.getSession().setAttribute("currentPage", String.valueOf(this.currentPage));
	...
	protected void refreshUrl() {
		...
		if (this.currentPage == 1)
			...
			buf.append("<a href='").append(this.path).append("&currentPage=1").append(parameter).append("' class='ls'>")
					.append("首页").append("</a>");
		// ////////////////////////#1157B7
		buf.append("&nbsp;&nbsp;&nbsp;&nbsp;");

		if (this.currentPage > 1) {
			buf.append("<a href='").append(this.path).append("&currentPage=").append(currentPage - 1).append(parameter)
					.append("' class='ls'>").append("上页").append("</a>");
		} else {
			buf.append("上页");
		...
		int currentSegment = this.currentPage % segment == 0 ? this.currentPage / segment
				: this.currentPage / segment + 1;

		/*
		 * for (int i = 1; i <= this.pageNumber; i++) { if (this.currentPage == i)
		 * buf.append("<font color='red'>").append(i).append("</font>");
		 * 
		 * else buf.append("<a href='").append(this.path).append(
		 * "&currentPage=").append(i).append(parameter).append(
		 * "' class='ls'>[").append(i).append( "]</a>"); }
		 */

		buf.append("&nbsp;&nbsp;");
		if (this.currentPage < this.pageNumber) {
			buf.append("<a href='").append(this.path).append("&currentPage=").append(currentPage + 1).append(parameter)
					.append("' class='ls'>").append("下页").append("</a>");
		} else {
		...
		if (this.currentPage == this.pageNumber)
			...
			buf.append("<a href='").append(this.path).append("&currentPage=").append(this.pageNumber).append(parameter)
					.append("' class='ls'>").append("末页").append("</a></font>&nbsp;&nbsp;");
		// ////////////////////
		...
		// }
		buf.append("<select onchange=\"javascript:window.location='").append(this.path).append("&currentPage='+")
				.append("this.options[this.selectedIndex].value").append(parameter).append("\">");
		for (int i = 0; i < this.pageNumber; i++) {
			if (this.currentPage == i + 1)
