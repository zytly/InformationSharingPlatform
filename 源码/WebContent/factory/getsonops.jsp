...
out.print(checkbox);
 ...
 if(ctype.equals("getlm")){
    String str = "";
    String uname = request.getParameter("uname"); 
    String month = request.getParameter("month"); 
	HashMap umap = new CommDAO().select("select * from sysuser where uname='"+uname+"'").get(0);
	String shdate = umap.get("shdate").toString().substring(0,7).replaceAll("-","");
	int ishdate = Integer.parseInt(shdate);
	int imonth = Integer.parseInt(month.replaceAll("-",""));
	 
	if(imonth<ishdate)
	{
	str = "N-用户开户日期为 "+umap.get("shdate").toString()+"，您输入的月份有误" ;
	}
	...
	str = "0" ;
	 ...
	 List<HashMap> list1 = new CommDAO().select("select * from bills where month='"+month+"' and uname='"+uname+"'");
	 if(list1.size()==1)
	 {
	 str = "N-月份 "+month+" 水表读数已登记" ;
	 }else{
	 
	 
	if(imonth>ishdate)
	{
	 String lmonth = Info.getDay(month+"-01", -2).substring(0,7);
	
	 List<HashMap> list = new CommDAO().select("select * from bills where month='"+lmonth+"' and uname='"+uname+"'");
	 if(list.size()==0)
	 {
	 str = "N-月份 "+lmonth+" 水表读数未登记，请先登记 "+lmonth+" 月份读数" ;
	 }else{
	 str = list.get(0).get("readnum").toString();
	 }
	}
	}
